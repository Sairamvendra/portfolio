...
// React Bits InfiniteMenu (WebGL2 sphere of draggable image discs), overlay UI
// restyled to the site's neobrutalism system with Tailwind instead of the
// reference CSS file. Core GL code is kept verbatim on purpose.
...
class InfiniteGridMenu {
  ...
  #init(onInit) {
    // alpha:true (unlike upstream) so the neobrutalism panel shows through
    // instead of an opaque black canvas
  ...
  #initTexture() {
    ...
    Promise.all(
      ...
      images.forEach((img, i) => {
        ...
        // center-crop to square: the atlas cell is square, so drawing the full
        // image would stretch anything non-square
        const s = Math.min(img.width, img.height);
        const sx = (img.width - s) / 2;
        const sy = (img.height - s) / 2;
        ctx.drawImage(img, sx, sy, s, s, x, y, cellSize, cellSize);
      });
...
export default function InfiniteMenu({ items = [], scale = 1.0 }) {
  ...
  // in once it snaps to an item (same states as upstream, Tailwind instead of CSS)
  const show = !isMoving;
    ...
    <div className="relative w-full h-full">
      ...
      {activeItem && (
        <>
          {activeItem.title && (
            <h3
              className={`hidden md:block absolute left-6 lg:left-10 top-1/2 -translate-y-1/2 z-10 -rotate-1 select-none pointer-events-none
                px-5 py-3 bg-neobrutalism-yellow border-3 border-neobrutalism-black shadow-neobrutalism-md
                text-2xl lg:text-3xl font-black uppercase max-w-[11em] break-words
                transition-all ${show ? 'opacity-100 duration-500' : 'opacity-0 -translate-x-2 duration-100'}`}
            >
              {activeItem.title}
            </h3>
          )}
          ...
          {activeItem.description && (
            <p
              className={`hidden md:block absolute right-6 lg:right-10 top-1/2 -translate-y-1/2 z-10 rotate-1 select-none pointer-events-none
                px-4 py-3 bg-neobrutalism-white border-3 border-neobrutalism-black shadow-neobrutalism-sm
                text-sm lg:text-base font-bold max-w-[24ch]
                transition-all ${show ? 'opacity-100 duration-500' : 'opacity-0 translate-x-2 duration-100'}`}
            >
              {activeItem.description}
            </p>
          ...
          <button
            ...
            className={`absolute left-1/2 -translate-x-1/2 bottom-6 z-10 grid place-items-center w-14 h-14
              bg-neobrutalism-yellow border-3 border-neobrutalism-black shadow-neobrutalism-md
              text-2xl font-black text-neobrutalism-black
              hover:shadow-neobrutalism-lg hover:-translate-y-0.5 active:translate-y-0 active:shadow-neobrutalism-sm
              focus:outline-none focus:ring-[3px] focus:ring-neobrutalism-black focus:ring-offset-2
              transition-all ${show ? 'opacity-100 duration-500' : 'opacity-0 translate-y-4 pointer-events-none duration-100'}`}
          >
            &#x2197;
          </button>
        </>
